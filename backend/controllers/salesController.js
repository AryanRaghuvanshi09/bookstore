  ...
  try {
    ...
    if (copies && Array.isArray(copies)) {
      for (const copyItem of copies) {
        ...
        copyDetails.push({ name: copy.name, quantity: copyQuantity, total: copy.price * requiredStock });
      }
    }
        ...
        $group: {
          ...
          copies: { 
            $push: {
              ...
              quantity: { $sum: '$copyDetails.quantity' },  // Sum of quantities per copy
              total: { $sum: '$copyDetails.total' },       // Sum of total price per copy (price * quantity)
            },
          },
        },
      },
